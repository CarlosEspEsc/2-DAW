    ...
    public partial class ProductosMantener : System.Web.UI.Page
    {
        ...
        protected void grdProductos_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string StrComandoSql = "SELECT IdProducto,DesPro,PrePro,IdUnidad,PRODUCTO.IdTipo,DesTip " +
                " FROM PRODUCTO INNER JOIN TIPO ON PRODUCTO.IdTipo = TIPO.IdTipo " + "WHERE PRODUCTO.IdProducto = '"
                + StrIdProducto + "';";
            using (SqlConnection conexion = new SqlConnection(StrCadenaConexion))
            {
                try {
                    ...
                    if (reader.HasRows)
                    {
                        reader.Read(); txtIdProducto.Text = reader.GetString(0);
                        ...
                        ddlIdTipo.SelectedItem.Selected = false; ddlIdTipo.SelectedItem.Text = reader.GetString(5);
                    }
                    else
                    {
        ...
        protected void btnInsertar_Click(object sender, EventArgs e)
        {
            ...
            Decimal dcPrecio; strIdProducto = txtIdProducto.Text; strDescripcion = txtDesPro.Text;
            dcPrecio = Convert.ToDecimal(txtPrePro.Text);
            strIdUnidad = ddlIdUnidad.SelectedItem.Text;
            strIdTipo = ddlIdTipo.SelectedItem.Value;
            string StrCadenaConexion = ConfigurationManager.ConnectionStrings["ConnectionString"].ConnectionString;
            string StrComandoSql = "INSERT PRODUCTO " + "(IdProducto,DesPro,PrePro,IdUnidad,IdTipo) VALUES (" +
            ...
            using (SqlConnection conexion = new SqlConnection(StrCadenaConexion))
            {
                try
                {
                    ...
                    else lblMensajes.Text = "Error al insertar el registro";
                    btnNuevo.Visible = true;
                    btnEditar.Visible = false;
                    btnEliminar.Visible = false;
                    btnInsertar.Visible = false;
        ...
        protected void btnEditar_Click(object sender, EventArgs e)
        {
            ...
            // Habilitar controles para edición, excepto txtIdProducto
            txtDesPro.Enabled = true;
            txtPrePro.Enabled = true;
            ddlIdUnidad.Enabled = true;
            ddlIdTipo.Enabled = true;
        ...
        protected void btnModificar_Click(object sender, EventArgs e)
        {
            ...
            // Cadena de conexión y comando SQL para actualizar el producto
            string StrCadenaConexion = ConfigurationManager.ConnectionStrings["ConnectionString"].ConnectionString;
            ...
            using (SqlConnection conexion = new SqlConnection(StrCadenaConexion))
            {
                try
                {
                    ...
                    comando.Parameters.AddWithValue("@DesPro", strDescripcion);
                    comando.Parameters.AddWithValue("@PrePro", FnComaPorPunto(dcPrecio));
            ...
            // Refrescar el GridView para mostrar los datos actualizados
            grdProductos.DataBind();
            grdProductos.SelectedIndex = -1;

            // Deshabilitar controles después de la edición
            FnDeshabilitarControles();
        }
        protected void btnEliminar_Click(object sender, EventArgs e)
        {
            ...
            // Verificar si hay una fila seleccionada en el GridView
            if (grdProductos.SelectedRow == null)
            {
                lblMensajes.Text = "Debe seleccionar un producto para eliminar.";
                return;
            }

            // Hacer visibles los botones de confirmación y cancelar
            btnNuevo.Visible = false;
            btnEditar.Visible = false;
            btnEliminar.Visible = false;
            btnInsertar.Visible = false;
        ...
        protected void btnBorrar_Click(object sender, EventArgs e)
        {
            ...
            // Cadena de conexión y comando SQL para eliminar el producto
            string StrCadenaConexion = ConfigurationManager.ConnectionStrings["ConnectionString"].ConnectionString;
            ...
            using (SqlConnection conexion = new SqlConnection(StrCadenaConexion))
            {
                try
                {
                    ...
                    if (inRegistrosAfectados == 1)
                    {
                        lblMensajes.Text = "Producto eliminado correctamente.";
                    }
                    else
                    {
                        lblMensajes.Text = "Error al eliminar el producto.";
                    }
                }
                catch (SqlException ex)
                {
